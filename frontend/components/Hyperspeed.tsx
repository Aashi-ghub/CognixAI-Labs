...
interface Distortion {
    ...
    getJS?: (progress: number, time: number) => any
}
    ...
    let colorArray: any[]
      ...
      colorArray = this.colors.map((c) => new THREE.Color(c))
		...
		for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
			...
			aMetrics.push(length)
			aMetrics.push(spd)
      const color = pickRandom<any>(colorArray)
			aColor.push(color.r)
			aColor.push(color.g)
			aColor.push(color.b)
			aColor.push(color.r)
...
class Road {
  ...
  island!: any
	...
	init() {
		this.leftRoadWay = this.createPlane(-1, this.options.roadWidth, true) as THREE.Mesh
		this.rightRoadWay = this.createPlane(1, this.options.roadWidth, true) as THREE.Mesh
		this.island = this.createPlane(0, this.options.islandWidth, false) as THREE.Mesh
	}
...
class App {
  ...
  clock: any
    ...
    // Render an opaque background color so the canvas looks like the demo
    const bgColor = new THREE.Color(options.colors.background)
