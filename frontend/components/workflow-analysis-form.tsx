...
export interface WorkflowAnalysisFormRef {
  open: () => void
}

type Step = 1 | 2 | 3

const WorkflowAnalysisForm = forwardRef<WorkflowAnalysisFormRef, {}>((props, ref) => {
  ...
  const [automationRequirements, setAutomationRequirements] = useState("")
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (!cancelled) {
        await fetchCompanySuggestions(q)
      }
    }, 200)
    return () => {
      cancelled = true
  ...
  const handleStep2Next = () => {
    if (!name.trim() || !industry || !teamSize || !challenge.trim() || !email.trim() || !phone.trim() || !automationRequirements.trim()) {
    ...
    try {
      const res = await fetch("/api/consultation", {
        ...
        body: JSON.stringify({ 
          ...
          automationRequirements: automationRequirements || null,
          ...
          <div className="space-y-4 animate-in fade-in-0 slide-in-from-bottom-4 duration-300">
            ...
            <div className="grid gap-3 max-h-[60vh] overflow-y-auto pr-2">
              <div className="grid gap-2">
                <Label htmlFor="name" className="text-white/90 text-sm">Name <span className="text-red-400">*</span></Label>
                <Input
                  id="name"
                  type="text"
                  value={name}
              ...
              <div className="grid gap-2">
                <Label htmlFor="companyWebsite" className="text-white/90 text-sm">Company Website (Optional)</Label>
                <Input
                  id="companyWebsite"
                  type="url"
              ...
              <div className="grid gap-2">
                <Label htmlFor="email" className="text-white/90 text-sm">Email <span className="text-red-400">*</span></Label>
                <Input
                  id="email"
                  type="email"
                  value={email}
              ...
              <div className="grid gap-2">
                <Label htmlFor="phone" className="text-white/90 text-sm">Phone Number <span className="text-red-400">*</span></Label>
                <PhoneInput
                  id="phone"
                  value={phone}
                  onChange={setPhone}
              ...
              <div className="grid gap-2">
                <Label htmlFor="automationRequirements" className="text-white/90 text-sm">
                  What automation do you require? <span className="text-red-400">*</span>
                </Label>
                <Textarea
                  id="automationRequirements"
                  value={automationRequirements}
                  onChange={(e) => {
                    setAutomationRequirements(e.target.value)
                    const ta = e.target as HTMLTextAreaElement
                    ta.style.height = "auto"
                    ta.style.height = ta.scrollHeight + "px"
                  }}
                  placeholder="Describe the automation you need (e.g., sales automation, customer support, data processing, etc.)"
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/50 resize-none min-h-[80px]"
                  rows={3}
                  required
                />
...
WorkflowAnalysisForm.displayName = "WorkflowAnalysisForm"
